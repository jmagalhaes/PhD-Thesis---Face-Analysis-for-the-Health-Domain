...
This chapter starts by explaining the importance of communication nowadays and the communication problems that exist in our societies, why communication skills are gaining more and more importance, not only on a personal level but also on an economical level and how \glspl{slp} work to prevent, diagnose and treat speech and language disorders.

In chapter~\ref{sec:speechAnatomy} and \ref{sec:faceAnatomy} an overview of the anatomy essential to communicate verbally as well as non-verbally is presented. Given the multimodality of communication, in chapter~\ref{sec:compcommunication} non-invasive methods are shown to detect facial activity as well as blood flow and other communication properties.
...
The coordination of articulatory movements involves different components such as \textit{articulators} (lips, tongue, teeth, jaw, hard and soft palate (velum)) used to vary the vocal cavity, \textit{resonating cavities} (nasal, oral, pharyngeal and soft palate) used to select and suppress overtones, \textit{the larynx and vocal chords} to produce sound waves and the \textit{lower respiratory tract} (trachea, lungs, diaphragm, bronchus, and bronchial tubes) necessary to supply the breath stream (see Fig.~\ref{fig:speechmuscles})\cite{PhonManual}. 
...
\begin{figure}
    \centering
    \includegraphics[width=10cm]{facs}
