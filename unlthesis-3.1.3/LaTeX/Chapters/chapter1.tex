...
The following chapter~\ref{cha:fundamentals}, explains the concept of communication and its complexity as well as speech disorders which can affect sound communication through failing speech motor control. Also an anatomic overview of the speech anatomy and facial muscles is given to comprehend the complexity of speech production. In chapter~\ref{cha:stateofart} it will be shown that in order to comprehend communication from a computational perspective, several modalities are necessary. It will be also shown how important affect detection has become over the years and how it can be used to understand more about motoric speech errors. The chapter ends with an outline of methods used for multimodal representation and data fusion.
Chapter~\ref{cha:methods} presents the proceedings to achieve the specified tasks in chapter~\ref{cha:plan}. 
